...
thpool_t* thpool_init(int threadsN){
    ...
    tp_p->readyThreadsN = 0;
    tp_p->readyThreadsN_mutex = (pthread_mutex_t*)malloc(sizeof(pthread_mutex_t));
    pthread_mutex_init(tp_p->readyThreadsN_mutex, NULL);
...
void thpool_thread_do(thpool_t* tp_p){
	...
	while(thpool_keepalive){
        ...
        pthread_mutex_lock(tp_p->readyThreadsN_mutex);
        tp_p->readyThreadsN++; 
        pthread_mutex_unlock(tp_p->readyThreadsN_mutex);
...
int thpool_add_work(thpool_t* tp_p, void *(*function_p)(void*), void* arg_p){
    ...
    pthread_mutex_lock(tp_p->readyThreadsN_mutex);
    if (tp_p->readyThreadsN == 0) 
    {
        printf("Created additional thread in pool \n");
        tp_p->threads = (pthread_t*)realloc(tp_p->threads, sizeof(pthread_t) * (tp_p->threadsN + 1));
        pthread_create(&(tp_p->threads[tp_p->threadsN]), NULL, (void *)thpool_thread_do, (void *)tp_p);
        tp_p->threadsN++;
    }
    pthread_mutex_unlock(tp_p->readyThreadsN_mutex);
...
void thpool_destroy(thpool_t* tp_p){
    ...
    pthread_mutex_destroy(tp_p->readyThreadsN_mutex);

	/* Dealloc */
    free(tp_p->readyThreadsN_mutex);
